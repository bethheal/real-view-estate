        ...
        <div className="max-w-7xl mx-auto p-4 md:p-8">
            <header className="text-center mb-12">
                ...
                <p className="text-lg text-gray-500">
                    Unlock unlimited listings, advanced lead management, and more.
                </p>
            </header>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                {SUBSCRIPTION_PLANS.map((plan) => (
                    <div 
                        key={plan.id}
                        className={`rounded-2xl border-4 ${plan.color} ${plan.bg} p-8 flex flex-col shadow-lg transition-all duration-300 hover:scale-[1.02]`}
                    >
                        {/* Tag/Header */}
                        {plan.tag && (
                        ...
                        <ul className="flex-grow space-y-3 mb-8">
                            ...
                            {plan.notIncluded.map((feature, index) => (
                                <li key={`no-${index}`} className="flex items-start text-gray-400 line-through">
                        ...
                        {/* Annual Savings Text */}
                         {billingCycle === 'annual' && plan.id !== 'basic' && (
                             <p className="mt-3 text-center text-sm text-gray-500">
                                 Save {plan.id === 'pro' ? 'GHS 500' : 'GHS 1200'} with Annual Billing!
                             </p>
                         )}
                    </div>
                ))}
            ...
            <footer className="mt-16 text-center text-gray-500">
                <p className="text-lg font-semibold mb-2">Secure Payments Powered By:</p>
                <div className="flex justify-center items-center gap-3">
                    {/* Replace with actual Paystack logo if you have it */}
                    <Shield size={20} className="text-green-500" />
                    <span className="font-bold text-gray-700">Paystack</span>
                    <p className="text-sm border-l ml-3 pl-3">Cancel anytime. 30-day money-back guarantee (Pro/Premium).</p>
                </div>
            </footer>
        </div>
    );
